...
if ($min % 1 == 0) {
    ...
    if (DB::num_rows()) {
        ...
        if ($massmail['send_partner'] != 1) {
            ...
            if ($massmail['users'] == 1) {
                $where = 'u.user_id != 0 AND u.lang IN (' . $massmail['languages'] . ') AND ';
            ...
            if ($massmail['users'] == 1 && $massmail['other'] == 1) {
                ...
                $where = '(u.user_id IS NULL OR (u.lang IN (' . $massmail['languages'] . ') ' . $whereAddon . ' ) ) AND ';
            }
        ...
        while ($row = DB::fetch_row()) {
            ...
            // UPDATE last_id
            $sql = "UPDATE massmail SET last_id=" . $row['id'] . " WHERE id=" . $massmail['id'];
