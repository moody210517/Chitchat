var CPages = function(guid) {
    ...
    this.replaceUrl = function(page){
        ...
        if (isSiteOptionActive('seo_friendly_urls')
            && ($this.pageType == 'photos' || $this.pageType == 'videos' || $this.pageType == 'friends')) {
            param = '';
            if (page && page != 1) {
                param = '/' + page;
            }
    ...
    this.isPageMyMediaList = function(type){//videos - photos
        ...
        if ($this.uid) {
            ...
            if(type=='videos')option='show_your_video_browse_videos';
            return isSiteOptionActive(option, 'edge_member_settings');
        }
    }

    this.myPageReload = function(type,noRedirect){
        type=type||$this.pageType;
        noRedirect=noRedirect||false;
        if(!type)return false;
        if($this.isPageMyMediaList(type)){
