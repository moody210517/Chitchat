$(function(){
    ...
    $('body, .menu_collapse a').not('.menu_audio_greeting_edge').on('click', closeMenuCollapse);
    ...
    if (isMobileSite) {
        ...
        function(e) {
            var $el=$(this);
            if($(e.target).is('.header_small_item_menu_name')){
...
function closeMenuCollapse(){
    $('.menu_collapse.in').collapse('hide');
}
...
function updateGridPhoto(){
    var $blGrid=$('#block_grid').addClass('grid_update'),
        $grid=$('#grid'),
        pageUrl=currentPage+currentPageParam,
        $blGridNew,
        $gridNew;
        ...
        if($blGridNew[0]){
            $blGridNew.imagesLoaded(function(){
                ...
                setTimeout(function(){
                    ...
                    }).delay(100).toggleClass('to_hide', 0);
                },10)
            })
        }
    })
