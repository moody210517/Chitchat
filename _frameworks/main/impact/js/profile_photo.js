...
*/
    ...
    this.showGallery = function(pid, uid, notLockedUser, isVideo, gender) {
        pid=pid*1;
        if(!pid)return;
        var pidInt=pid;
        if(isVideo){
    ...
    this.openGalleryOnTheWallVideo = function(pid, uid, notLockedUser, startParams) {
        $this.startVideoParams=startParams || {};
        $('#pp_gallery_photos_content').empty().html($this.sourceGalleryHtml);
        setTimeout(function(){$this.showGallery(pid, uid, notLockedUser,  true)},100);
    }
    ...
    this.openGalleryId = function(pid) {
        $('#pp_gallery_photos_content').empty().html($this.sourceGalleryHtml);
        var s=''+pid;
        if(s.indexOf('v_')!==-1){
