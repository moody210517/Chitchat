...
class CBlogsTools
{
    ...
    static public function getListBlogs($limit = '0,6', $typeOrder = '')
	{
        if ($typeOrder == 'order_popular_bloggers') {
            //return CBlogsTools::getPopularBloggers($limit, true);
            $sql = 'SELECT B.*, U.name, U.name_seo, U.country, U.city
                      FROM `user` AS U
                      JOIN `blogs_post` AS B on B.id = (SELECT id FROM `blogs_post` WHERE user_id = U.user_id ORDER BY dt DESC LIMIT 1)
                     WHERE U.blog_posts > 0
                  ORDER BY U.blog_visits DESC, U.blog_comments DESC, U.blog_posts DESC
                     LIMIT ' . $limit;
        } else {
            $orderBy = 'B.dt DESC';
        ...
        foreach ($posts as $k => $post) {
            ...
            $post['text_short'] = trim(self::_filterRemoveEmptyLines(self::_filterRemoveUnusedTags(self::_filterLinksTagsToHtml(neat_trim($post['text'], $maxLenTextShort)))));
            $rows[] = $post;
        }
        return $rows;
    }
    ...
    static public function uploadImg($post_id, $img_id, $file)
    {
        ...
        if ($im->loadImage($file)) {
            ...
            //to post column
            $im->loadImage($file);
            if ($im->getWidth() > self::$videoWidth) {
                $im->resizeW(self::$videoWidth);
                $im->saveImage($filePrefix . "_s.jpg", $g['image']['quality']);
            } else {
                copy($file, $filePrefix . "_s.jpg");
            }

            //original
            copy($file, $filePrefix . "_o.jpg");
            $path = array($filePrefix . '_t.jpg', $filePrefix . '_m.jpg', $filePrefix . '_s.jpg', $filePrefix . "_o.jpg");
    ...
    /*static public function cSI()
    {
        $posts = DB::select('blogs_post');
        foreach ($posts as $p) {
            DB::update('blogs_post', array('search_index' => self::_filterSearchIndex(self::_filterRemoveUnusedTags($p['text']))), "id='" . $p['id'] . "'");
        }
    }
    static public function cThumbs()
    {
        global $g;
        $d = dir($g['path']['dir_files'] . "blogs/");
        while (false !== ($entry = $d->read())) {
            $im = new Image();
            if (substr($entry, -6) == '_o.jpg') {
                $im->loadImage($d->path . $entry);
                $file = substr($d->path . $entry, 0, strlen($d->path . $entry) - 6) . '_t.jpg';
                $im->resizeCroppedMiddle(85, 64);
                $im->saveImage($file, 80);
            }
        }
        $d->close();
        die('ok');
    ...
    static public function deleteImg($post_id, $img_id)
    {
        ...
        if (file_exists($filePrefix . "_t.jpg")) unlink($filePrefix . "_t.jpg");
        if (file_exists($filePrefix . "_s.jpg")) unlink($filePrefix . "_s.jpg");
        if (file_exists($filePrefix . "_m.jpg")) unlink($filePrefix . "_m.jpg");
        if (file_exists($filePrefix . "_o.jpg")) unlink($filePrefix . "_o.jpg");
    }
    ...
    static public function existsImg($post_id, $img_id)
    {
        global $g;
        $filePrefix = $g['path']['dir_files'] . "blogs/" . $post_id . '_' . $img_id;
        return (file_exists($filePrefix . "_s.jpg") and file_exists($filePrefix . "_m.jpg") and file_exists($filePrefix . "_o.jpg") and file_exists($filePrefix . "_t.jpg"));
    }
	...
	static public function delPostById($id)
	{
        ...
        $post = DB::one('blogs_post', "id='" . $id . "'");
        if (is_array($post)) {
            self::deleteImgs($id, explode("|", $post['images']));
            DB::delete('blogs_post', "user_id='" . guser('user_id') . "' AND id='" . $id . "'");

            $sql = 'SELECT id FROM blogs_comment
                WHERE post_id = ' . to_sql($id, 'Number');
            $comments = DB::rows($sql);

            if(is_array($comments) && count($comments)) {
                foreach($comments as $comment) {
                    self::delCommentByIdByAdmin($comment['id']);
                }
            }

            DB::update('user', array('blog_posts' => new DBNoEsc('blog_posts-1')), "user_id='" . guser('user_id') . "'");
            if($post['user_id'] == guid()) {
                Wall::remove('blog_post', $id, 0);
            }

            Wall::removeBySiteSection('blog', $id);
        }
    }
	...
	static protected function _filterImgTagsToDb($text, $id)
    {
	...
	static protected function _filterPostToHtml($row, $withUser = false)
	{
        ...
        if(trim($row['subject_req']=="") && $p!="blogs_write.php") $row['subject_req']= isset($l['all']['no_subject']) ? $l['all']['no_subject'] : "No subject";
        ...
        if (pl_strlen($stext) >  $maxlen) {
            $row['text_short'] = trim(self::_filterRemoveEmptyLines(self::_filterRemoveUnusedTags(self::_filterLinksTagsToHtml(neat_trim($row['text'], $maxlen)))));
            $row['text_short_readable'] = nl2br($row['text_short']);
            $row['text_is_short'] = false;
        } else {
            $row['text_short'] = self::_filterRemoveUnusedTags($row['text']);
...
*/
	...
	static public function delCommentById($id)
	{
        ...
        if (is_array($comment)) {
            DB::delete('blogs_comment', "user_id='" . guser('user_id') . "' AND id='" . $comment['id'] . "'");
	...
	static public function delCommentByIdByAdmin($id)
	{
        $comment = self::getCommentById($id);
        if (is_array($comment)) {
            DB::delete('blogs_comment', "id='" . $comment['id'] . "'");
            $post = self::getPostById($comment['post_id']);
            if (is_array($post)) {
                DB::update('user', array('blog_comments' => new DBNoEsc('blog_comments-1')), "user_id='" . $post['user_id'] . "'");
                DB::update('blogs_post', array('count_comments' => new DBNoEsc('count_comments-1')), "id='" . $post['id'] . "'");
    ...
    static public function getTypeOrderList($notRandom = false, $lang = false)
	{
        ...
        $list = array(
            'order_popular_bloggers' => l('order_popular_bloggers', $lang),
