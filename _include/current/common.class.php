   ...
   static function getLoginPage()
    ...
    static function urlSiteSubfolders()
    {
        ...
        if(isset($_SERVER['CONTEXT_PREFIX']) && $_SERVER['CONTEXT_PREFIX'] !== '' && isset($_SERVER['CONTEXT_DOCUMENT_ROOT']) && strpos($_SERVER['REQUEST_URI'], $_SERVER['CONTEXT_PREFIX']) !== false) {
            ...
            $urlSubfolders = str_replace(rtrim($_SERVER['DOCUMENT_ROOT'], '/'), '', $baseDir);

            $serverDocumentRootParts = explode('/', $_SERVER['DOCUMENT_ROOT']);
            ...
            if($commonFolders) {
                ...
                if(strlen($_SERVER['DOCUMENT_ROOT']) === strpos($_SERVER['DOCUMENT_ROOT'], $commonPath) + strlen($commonPath) && strpos($baseDir, $commonPath) === 0) {
    ...
    static function getIdsByNear($cityInfo,$howNear=0)
        {
            $eq=array('=','=','=');
            if($howNear>0){
                $eq[0]='!=';
    ...
    static function sqlUsersNearCity($cityInfo, $limit = 5)
    {
        ...
        while($countUsersNear<$limit && $nNear<=3){
            if(!isset($cacheArr[$nNear])){
                $cacheArr[$nNear]=self::getIdsByNear($cityInfo,$nNear);
                $updateCache=true;
            }
...
JS;
    ...
    static function getSeoSite($url, $uid, $userInfo = null, $noPlaceCity = false)
    {
        global $g, $p;
    ...
    static function parseSeoSite(&$html)
    {
        ...
        if (isset($g['main']['description'])) {
            ...
            $uid = guid();
            if (($p == 'search_results.php' && get_param('display') == 'profile')
                || $p == 'profile_view.php') {
                $url = 'profile';
                if ($mobile) {
    ...
    static public function pageUrl($page, $uid = null, $id = null)
    {
        global $g, $g_user;
        ...
        if(Common::isOptionActive('seo_friendly_urls')) {
            ...
            if ($optionTmplName == 'edge') {
                ...
                if (isset($customUrls[$page])) {
                    ...
                    $assignProfilePage = array('user_photos_list', 'user_vids_list', 'user_friends_list',
                                               'my_friends_online');
            ...
            $urls = array(
                ...
                'group_add'           => 'groups_group_add.php'
                /* Edge */
            );
    ...
    static function isPage($page, $isMyProfile = false)
    {
        ...
        } elseif ($page == 'user_vids_list') {
            $isPage = $p == 'vids_list.php' && $paramUid;
        } elseif ($page == 'user_my_vids_list') {
            $isPage = $p == 'vids_list.php' && $guid == $paramUid;
        } elseif ($page == 'vids_list') {
            $isPage = $p == 'vids_list.php' && !$paramUid;
        } elseif ($page == 'user_photos_list') {
            $isPage = $p == 'photos_list.php' && $paramUid;
        } elseif ($page == 'user_my_photos_list') {
            $isPage = $p == 'photos_list.php' && $guid == $paramUid;
