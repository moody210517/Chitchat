    ...
    function parseBlockEdge(&$html)
    {
        ...
        if ($itemsPreparedWhere) {
            ...
            if ($isProfileWall) {
                ...
                $where .= ' AND (u.user_id = ' . $guidSql .  ' OR (1=1 AND ubl1.id IS NULL AND ubl2.id IS NULL)) AND ((ucom1.user_id = ' . $guidSql .
                           ' OR (1=1 AND ubl1.id IS NULL AND ubl2.id IS NULL))  OR (ucom2.user_id = ' . $guidSql . ' OR (1=1 AND ubl1.id IS NULL AND ubl2.id IS NULL)))';

                $sql = 'SELECT w.*
                          FROM `wall` AS w
                          LEFT JOIN user AS u ON u.user_id = w.user_id
                          LEFT JOIN userinfo AS i ON u.user_id = i.user_id
                          LEFT JOIN user_block_list AS ubl1 ON (ubl1.user_to = u.user_id AND ubl1.user_from = 451)
                          LEFT JOIN user_block_list AS ubl2 ON (ubl2.user_from = u.user_id AND ubl2.user_to = 451)
        ...
        if ($itemsUpdate) {
            foreach ($itemsUpdate as $k1 => $row) {
                ...
                if ($row['last_action_like'] != $items[$row['id']]['like']) {
                    $likeUpdated[$row['id']] = array('date_like' => $row['last_action_like'], 'count' => $row['likes']);
        ...
        if ($commentUpdated) {
            foreach ($commentUpdated as $key => $value) {
                ...
                if ($commentsOfItem) {
                    ...
                    //print_r_pre($commentsPreparedPhoto);
                    //var_dump_pre($commentsPrepared);
                    //var_dump_pre($commentsPreparedPhoto);
                    //var_dump_pre($commentsPreparedVideo);
                    //exit();
                    ...
                    foreach ($data as $k => $v) {
                        ...
                        //var_dump_pre($commentsNotExists, true);
