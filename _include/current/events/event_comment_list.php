...
class CEventsEventCommentList extends CHtmlBlock
{
	...
	function parseBlock(&$html)
	{
        ...
        if($event)
        {
            ...
            $html->setvar('event_title', strcut(to_html($event['event_title']), 24));
            $html->setvar('event_title_full', to_html($event['event_title']));

        	$sql_base = CEventsTools::comments_by_event_sql_base($event['event_id']);

	        $n_results = CEventsTools::count_from_sql_base($sql_base);

	        $page = intval(get_param('event_comment_list_page', 1));
	        ...
	        $comments = CEventsTools::retrieve_from_sql_base($sql_base, $n_results_per_page, ($page - 1) * $n_results_per_page);
	        ...
	        if(count($comments))
	        {
		        ...
		        foreach($comments as $comment)
		        {
		            ...
		            $html->setvar('comment_text', CEventsTools::filter_text_to_html($comment['comment_text']));
	                ...
	                if($event['user_id'] == $g_user['user_id'] || $comment['user_id'] == $g_user['user_id'])
	                ...
	                $comment_comments = CEventsTools::retrieve_from_sql_base(CEventsTools::comments_by_comment_sql_base($comment['comment_id']));
	                foreach($comment_comments as $comment_comment)
	                {
                        $html->setvar('comment_comment_id', $comment_comment['comment_id']);
                        $html->setvar('comment_comment_text', CEventsTools::filter_text_to_html($comment_comment['comment_text'], true, $thumbnail_postfix, false));
	                    ...
	                    if($event['user_id'] == $g_user['user_id'] || $comment_comment['user_id'] == $g_user['user_id'] || $comment['user_id'] == $g_user['user_id'])
