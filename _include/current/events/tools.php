...
*/
    ...
    static function events_by_calendar_day($day_time)
    {
        global $g_user;

    	$order_by_from_settings = self::order_by_from_settings();

        $sql = "events_event as e LEFT JOIN events_event_guest as eg ON e.event_id = eg.event_id, geo_city as c WHERE c.city_id = e.city_id " .
            "AND eg.user_id=" . $g_user['user_id'] . " AND e.event_datetime >= '" . date("Y-m-d", $day_time) . "' AND e.event_datetime < DATE_ADD('" . date("Y-m-d", $day_time) . "', INTERVAL 1 DAY) " .
            " ORDER BY e.event_datetime ASC, " . ($order_by_from_settings ? ($order_by_from_settings . "") : '');
    ...
    static function do_upload_event_image($name, $event_id, $time = false, $addOnWall = true)
    {
        global $g;
        global $g_user;
