...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        ...
        foreach ($this->error_messages as $key => $item) {
            $this->error_messages[$key] = l($item);
        }
        if ($error_messages) {
    ...
    protected function get_error_message($error) {
        $key = array_key_exists($error, $this->error_messages) ?
            $this->error_messages[$error] : $error;

        if ($key == 'max_file_size') {
            $error_msg = lSetVars('max_file_size', array('size'=>$this->options['max_file_size']/1048576));
        } else {
            $error_msg = l($key);
        }
    ...
    public function post($print_response = true) {
        ...
        if ($this->options['file_name_save'] == '') {
            ...
            $file_name = $this->options['file_name_save'];
        }
