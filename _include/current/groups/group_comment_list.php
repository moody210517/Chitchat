...
class CGroupsGroupCommentList extends CHtmlBlock
{
	...
	function parseBlock(&$html)
	{
        ...
        if($group)
        {
            ...
            $html->setvar('group_title', strcut(to_html($group['group_title']), 24));
            $html->setvar('group_title_full', to_html(he($group['group_title'])));

        	$sql_base = CGroupsTools::comments_by_group_sql_base($group['group_id']);

	        $n_results = CGroupsTools::count_from_sql_base($sql_base);

	        $page = intval(get_param('group_comment_list_page', 1));
            ...
            if($group['user_id'] == $g_user['user_id'] || CGroupsTools::is_group_member($group['group_id']))
            {
                ...
                $html->parse('new_comment_form_top');
			...
			$i=0;
				...
				if($i++==0) $html->setvar('posted_comment_id', $comment['comment_id']);
	                ...
	                $html->setvar('comment_user_name_full', to_html(he($comment['name'])));
	                ...
	                $comment_comments = CGroupsTools::retrieve_from_sql_base(CGroupsTools::comments_by_comment_sql_base($comment['comment_id']));
