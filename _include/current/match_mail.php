...
class Match_Mail {
    ...
    static function lastReceiverId()
    {
        $sql = 'SELECT user_id FROM user
            WHERE active = 1 AND match_mail != 2
    ...
    static function run($debug = false)
    {
        ...
        if($todayStart && $lastActionDate == $todayDate && self::getOption('status') == '2') {
            self::debugger('TODAY START and STATUS STOP');
            return;
        }

        if (!$todayStart && self::getOption('status') == '2') {
            self::debugger('TODAY NOT START and STATUS STOP');
            return;
        }

        if (self::getOption('status') == '1' && self::getLastReceiverId() <= self::getOption('last_receiver_id')) {
