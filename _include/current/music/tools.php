...
class CMusicTools {
    ...
    static function generate_upload_name() {
        ...
        } while (file_exists($g['path']['dir_files'] . "music/tmp/" . $upload_name . ".mp3"));
    ...
    static function do_upload_song_image($name, $song_id, $time = false) {
        global $g;
        global $g_user;
        ...
        if (isset($_FILES[$name]) && is_uploaded_file($_FILES[$name]["tmp_name"])) {
            ...
            if ($im->loadImage($_FILES[$name]['tmp_name'])) {
                #Old settings $g['music_song_image']['big_x'], $g['music_song_image']['big_y']
                $im->resizeWH($im->getWidth(), $im->getHeight(), false, $g['image']['logo'], $g['image']['logo_size']);
                $im->saveImage($sFile_ . "b.jpg", $g['image']['quality']);
                @chmod($sFile_ . "b.jpg", 0777);
            }
...
<script type="text/javascript">
    ...
    </script><div style="width:{$player_lx};height:{$player_ly}" id="music_flash_player_{$song_index}"><a href="http://www.adobe.com/go/getflashplayer"><img src="http://www.adobe.com/images/shared/download_buttons/get_flash_player.gif" alt="Get Adobe Flash player"></a></div>
EOT;
