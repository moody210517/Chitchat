...
*/
	Function StoreAccessToken($access_token)
	{
		...
		if(!$this->GetAccessTokenURL($access_token_url))
			return false;
        if(function_exists('set_session')){
            set_session('OAUTH_ACCESS_TOKEN_'.$access_token_url,$access_token);
        }    
...
*/
	Function GetAccessToken(&$access_token)
	{
		...
		if(!$this->GetAccessTokenURL($access_token_url))
			return false;
        if(function_exists('get_session')){
            $token=get_session('OAUTH_ACCESS_TOKEN_'.$access_token_url);
            if($token){
                $_SESSION['OAUTH_ACCESS_TOKEN'][$access_token_url] = $token;
            }    
        }    
...
*/
	Function ResetAccessToken()
	{
        ...
        if(function_exists('delses')){
            delses('OAUTH_ACCESS_TOKEN_'.$access_token_url);
            delses('OAUTH_STATE');
        }    
		return true;
	}
