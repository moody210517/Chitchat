...
class Securimage {
  ...
  function saveData()
  {
        setses('securimage_code_value', strtolower($this->code));
  }
  ...
  {
        $securimageCodeValue = get_session('securimage_code_value');
        if (!empty($securimageCodeValue)
            && $securimageCodeValue == strtolower(trim(self::$code_entered))) {
            self::$correct_code = true;
            setses('securimage_code_value', '');
        } else {
            self::$correct_code = false;
        }
  }
