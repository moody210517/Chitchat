...
function delete_user($user_id)
{
	...
	foreach($ids as $id) CBlogsTools::delPostByIdByAdmin($id, true);

	$ids = DB::column("SELECT id FROM blogs_comment WHERE user_id=" . to_sql($user_id, "Number") . "");
	foreach($ids as $id)  CBlogsTools::delCommentByIdByAdmin($id);
	...
	$ids = DB::column("SELECT event_id FROM events_event WHERE user_id=" . to_sql($user_id, "Number") . "");
        ...
        if(CProfilePhoto::createBasePhotoFile($im, $sFile_, $file)) {
            ...
            if(!$isCity && Common::isOptionActive('nudity_filter_enabled')) {
                ...
                if($nudityFilter->isPorn()) {
                    if($vis !== 'P') {
                        DB::update($table, array('visible' => 'Nudity'), 'photo_id = ' . $photo_id);
...
function deletephoto($user_id, $photo_id)
{
	global $g;
...
{
	global $g;
	$photos = DB::rows("SELECT photo_id FROM photo WHERE user_id=" . to_sql($user_id, "Number"));
    if($photos) {
        foreach($photos as $photo) {
            deletephoto($user_id, $photo['photo_id']);
        }
    }
}
