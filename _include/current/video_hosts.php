...
class VideoHosts
{
    ...
    static function embedInfo($id, $type)
    {
        ...
        if($type == 'youtube') {
            ...
            if(!$info) {
                $info = array();
                $url = 'http://www.youtube.com/oembed?url=youtu.be/' . $id . '&format=xml';
    ...
    static protected function _filterFromDbYoutube($text, $pretag, $posttag, $w)
    {
        ...
        $htmlUrlTemplate = '<br><a class="embed_video_url" target="_blank" href="http://www.youtube.com/watch?v={id}">{title}</a><br>';
        ...
        foreach ($objs as $obj) {
            ...
            if(self::getMobile()) {
                ...
                $html = '<a target="_blank" href="http://www.youtube.com/watch?v=' . $obj . '"><img width="240" src="' . self::_filterOneImageBigFromDbYoutube($obj, 1, 'mq') . '"></a>' . $htmlUrl;
            }
            $tag = '{youtube:' . $obj . '}';
