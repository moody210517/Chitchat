...
class CVidsTools
{
    ...
    static function addWherePrivateCondition($where = '')
    {
        if(!defined('ADMINISTRATOR')) {
            ...
            $where .= $and . ' (private = 0) OR (private = 1 AND user_id = ' . to_sql(guid(), 'Number') . ') ';
        }
	...
	{
        $where = self::addWherePrivateCondition($where);
        return self::_filterVideosToHtml(DB::select('vids_video', $where, $order, $limit, '*, count_comments + count_comments_replies AS count_comments_all'));
    }
    ...
    {
        return DB::count('vids_video', self::addWherePrivateCondition());
    }
	...
	static public function getVideosByRand($limit = '0,10')
	{
        ...
        {
            return DB::count('vids_video','','RAND()');
        }
	...
	static public function countVideosByQuery($q)
	{
        ...
        $q = DB::esc_like($q);
        return DB::count('vids_video', "search_index LIKE '% " . $q . " %'");
    }
	static public function getVideosByQuery($q, $limit = '0,10')
	{
        ...
        $videos = DB::select('vids_video', "search_index LIKE '% " . $q . " %'", 'dt DESC', $limit);
        $videos_r = array();
	...
	static public function insertCommentByVideoId($video_id, $text = null, $date = null, $isAddWall = true, $videoUserId = null, $getInfo = false)
	{
        ...
        if (is_array($video)) {
            ...
            $commentUserId = 0;
            if ($videoUserId === null) {
                $videoUserId = DB::result('SELECT `user_id` FROM `vids_video` WHERE `id` = ' . to_sql($video_id));
            }
            if ($parentId) {
	...
	static public function delCommentByIdByAdmin($id)
	{
        ...
        if (is_array($comment)) {
            ...
            if (is_array($video)) {
                ...
                DB::update('vids_video', array('count_comments' => $comments), "id='" . $video['id'] . "'");
            }
        }
    }
    ...
    static public function increaseViewCountVideoById($id, $user_id)
	{
        $responseData = false;
        if (guid()) {
            //DB::update('vids_video', array('count_views' => new DBNoEsc('count_views+1')), 'id=\'' . intval($id) . '\'');
	...
	{
        $video = DB::one('vids_video', "id='" . intval($id) . "'");
        if (is_array($video)) {
	...
	static public function insertVideo($file="", $active=1, $isAddData = true, $isUploaded = 0)
	{
        ...
        if (self::validateVideoCode()) {
            DB::update('vids_video', array('code' => VideoHosts::filterOneToDb(par('code'))), 'id="' . $id . '"');
        ...
        if($file!=""){
            if (VideoUpload::upload($id, $file)) {
                DB::update('vids_video', array('code' => 'site:' . $id . ''), 'id="' . $id . '"');
                if ($isAddData) {
                    DB::update('user', array('vid_videos' => new DBNoEsc('vid_videos+1')), 'user_id=' . guser('user_id'));
                    Wall::add('vids', $id);
                }
            } else {
                DB::delete('vids_video', 'id="' . $id . '"');
                return;
            }
        }
	...
	static public function updateVideoById($id, $isAdmin = false)
	{
        ...
        DB::update('vids_video', $row, 'id=\'' . intval($id). '\'' . ($isAdmin ? '' : ' AND user_id=\'' . guser('user_id') . '\''));
        ...
        if (is_array($video)) {
            ...
            if (is_array($isRated)) {
                if ($video['count_rates'] > 0) {
                    ...
                    DB::update('vids_video', array('rating' => $newRate), "id='" . $id . "'");
                } else {
                    $newRate = $rate;
                    DB::update('vids_video', array('rating' => $newRate, 'count_rates' => new DBNoEsc('count_rates+1')), "id='" . $id . "'");
                }
            } else {
	...
	static public function delVideoById($id, $isAdmin = false)
	{
        ...
        if (is_array($video)) {
            ...
            DB::delete('vids_video', "id='" . $id . "'" . ($isAdmin ? '' : ' AND user_id=\'' . guser('user_id') . '\''));
            Wall::remove('vids', $id);
            ...
            DB::update('user', array('vid_videos' => new DBNoEsc('vid_videos-1')), "user_id='" . $video['user_id'] . "' AND vid_videos > 0");
...
class VideoUploadFfmpeg
{
    ...
    static function convert($video_file, $output_video, $output_img)
    {
        ...
        $cmd = $ffmpeg . ' -i ' . $video_file . $strictExperimental . ' -movflags +faststart -c:a copy -crf 29 -vf "scale=min(iw*720/ih\,1280):min(720\,ih*1280/iw), pad=1280:720:(1280-iw)/2:(720-ih)/2" ' . $output_video;
        exec($cmd);
...
class VideoUpload
{
    ...
    static function delete($id)
    {
            ...
            if (file_exists($file . '.mp4')) unlink($file . '.mp4');
            if (file_exists($file . '.flv')) unlink($file . '.flv');
            if (file_exists($file . '.jpg')) unlink($file . '.jpg');
            //if (file_exists($file . '_s.jpg')) unlink($file . '_s.jpg');
            if (file_exists($file . '_b.jpg')) unlink($file . '_b.jpg');
            if (file_exists($file . '_src.jpg')) unlink($file . '_src.jpg');
            /*if (file_exists($file . '._m.jpg')) unlink($file . '.txt');*/
    }
    ...
    static function updateFromString()
    {
        ...
        $sql = 'SELECT `user_id` FROM `vids_video` WHERE `id` = ' . to_sql($id);
