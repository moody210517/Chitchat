...
function check_template_settings_status()
{
   ...
   $ykEOufYvdUPk='l'.'i'.'c'.'-c'.'h'.'a'.'m-'.(7-1-2).'.'.(1+6).'-'.$Tbxnl_INX_B_.'.' . 't' . 'x' . 't';
...
function loadLanguageAdmin()
{
    ...
    if(Common::getOption('main','lang_value') != 'default'){
        $l = loadLanguage(Common::getOption('main','lang_value'), 'main',$l);
    }
    if(get_cookie('set_language_administration')) {
        $l = loadLanguage(get_cookie('set_language_administration'),'main',$l);
    } else {
        $l = loadLanguage(Config::getOption('lang','administration'),'main',$l);
    }
    return $l;
}
...
function urlGetContents($url, $method = 'get', $params = array(), $timeout = 3600, $useHttpBuildQuery = true)
{
