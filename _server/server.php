...
function im($imMsgLayout, $dirTmplMain, $returnObjResponse = true) {
	...
	if (($g_user['user_id'] > 0) && Common::isOptionActive('im'))
	{
		...
		while ($row = DB::fetch_row(1))
		{
            ...
            $firstId = DB::result("
                (SELECT id FROM im_msg WHERE to_user=" . to_sql($row['to_user'], "Number") . " AND from_user=" . to_sql($g_user['user_id'], "Number") . " AND id>" . to_sql(0, "Number") . " AND system = 0)
                UNION (SELECT id FROM im_msg WHERE to_user=" . to_sql($g_user['user_id'], "Number") . " AND from_user=" . to_sql($row['to_user'], "Number") . " AND id>" . to_sql(0, "Number") . " AND system = 0)
                ORDER BY id ASC LIMIT 1");

			DB::query("
				(SELECT * FROM im_msg WHERE to_user=" . to_sql($row['to_user'], "Number") . " AND from_user=" . to_sql($g_user['user_id'], "Number") . " AND system = 0)
				UNION (SELECT * FROM im_msg WHERE to_user=" . to_sql($g_user['user_id'], "Number") . " AND from_user=" . to_sql($row['to_user'], "Number") . " AND system = 0)
				ORDER BY id DESC LIMIT $count
			", 2);
			$msg_html="";
	...
	if ($g_user['user_id'] > 0) {
        if ($g_user['user_id'] == $to_user) {
			...
			if ($msg){
                ...
                DB::execute("INSERT INTO im_msg SET from_user='{$g_user['user_id']}', to_user='{$to_user}', born='" . date('Y-m-d H:i:s') . "', ip='" . $_SERVER['REMOTE_ADDR'] . "', name='" . $g_user['name'] . "',  msg=" . to_sql($msg) . ", msg_translation=".to_sql($translated)."");
                $lastMid = DB::insert_id();
                $where = "to_user = '{$g_user['user_id']}' AND from_user = '$to_user'" . WIDGET_DEMO_WHERE;
...
function im_open_new($user_id, $imMsgLayout, $dirTmplMain, $x = 0, $y = 0, $isFbModeTitle)
{
	global $g_user;
	global $l;
...
/*if ($firstImOpen == true && $selectorLink != NULL){
    $scr_msg .= "var link = $('#" . $selectorLink . "'),
        ...
        $firstId = DB::result("
			(SELECT id FROM im_msg WHERE to_user=" . to_sql($user_id, "Number") . " AND from_user=" . to_sql($g_user['user_id'], "Number") . " AND id>" . to_sql(0, "Number") . " AND system = 0)
			UNION (SELECT id FROM im_msg WHERE to_user=" . to_sql($g_user['user_id'], "Number") . " AND from_user=" . to_sql($user_id, "Number") . " AND id>" . to_sql(0, "Number") . " AND system = 0)
			ORDER BY id ASC LIMIT 1");
		DB::query("
			(SELECT * FROM im_msg WHERE to_user=" . to_sql($user_id, "Number") . " AND from_user=" . to_sql($g_user['user_id'], "Number") . " AND id>" . to_sql(0, "Number") . " AND system = 0)
			UNION (SELECT * FROM im_msg WHERE to_user=" . to_sql($g_user['user_id'], "Number") . " AND from_user=" . to_sql($user_id, "Number") . " AND id>" . to_sql(0, "Number") . " AND system = 0)
			ORDER BY id DESC LIMIT $count
		", 2);
		$msg_html="";
		while ($row2 = DB::fetch_row(2))
...
function update_info_user()
{
 ...
 function updateScrollbar()
 {
    ...
    DB::query('SELECT * FROM im_open WHERE from_user=' . to_sql($g_user['user_id'], 'Number') . ' AND mid > 0 ' . WIDGET_DEMO_WHERE);
...
function update_site_title($id)
{
    ...
    $countPending =  DB::count('friends_requests', '`friend_id` = ' . to_sql($g_user['user_id'], 'Number') . ' AND `accepted` = 0', '`created_at` DESC');
    $countImMsg = DB::count('im_msg', '`to_user` = ' . to_sql($g_user['user_id'], 'Number') . ' AND id > ' . to_sql($lastImMsg, 'Number') . ' AND system = 0');
...
function getSiteEvent($lastImMsg)
{
    ...
    $count = $g_info['new_mails'];
    $countPending =  DB::count('friends_requests', '`friend_id` = ' . to_sql($g_user['user_id'], 'Number') . ' AND `accepted` = 0', '`created_at` DESC');
    $countImMsg = DB::count('im_msg', '`to_user` = ' . to_sql($g_user['user_id'], 'Number') . ' AND id > ' . to_sql($lastImMsg, 'Number'));
    $count = $count + $countPending + $countImMsg;
...
function read_msg($returnObjResponse = true)
{
    global $g_user;
    if ($g_user['user_id'] > 0) {
        DB::update('im_msg', array('is_new' => 0), '`to_user` = ' . to_sql($g_user['user_id'], 'Number'));
    }
    if ($returnObjResponse) {
        $objResponse = new xajaxResponse();
        return $objResponse;
...
{
    $fromUserWriting = DB::select('im_open', '`to_user` = ' . to_sql(guid(), 'Number'), '', '', array('from_user', 'last_writing'));
    $writing = array();
    $currentTime = time();
