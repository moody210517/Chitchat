...
var Player = (function (_Component) {
  ...
  Player.prototype.handleTechDblClick_ = function handleTechDblClick_(event) {
    if (event.button !== 0) return;
    $doc.find(".vjs-fullscreen-control").click();
  };
...
var Slider = (function (_Component) {
  ...
  Slider.prototype.update = function update() {
    ...
    var per = (progress * 100).toFixed(2),
        percentage = per + '%';
    // Set the new bar width or height
    if (this.vertical()) {
        bar.el().style.height = percentage;
    } else {
        if ($doc.find('.vjs-playing')[0]) {
            if (per > 99) {
                bar.el().classList.remove('animate');
            } else {
                bar.el().classList.add('animate');
            }
        } else {
            bar.el().classList.remove('animate');
        }
