	...
	function parseBlock(&$html)
	{
		...
		for($friend_n = 0; $friend_n != 4; ++$friend_n)
		{
			if($friend_row = DB::fetch_row())
			{
				...
				if ($row['photo_id'] != "" and file_exists($g['path']['dir_files'] . "photo/" . $row['user_id'] . "_" . $row['photo_id'] . "_r.jpg")) $row['photo'] = "photo/" . $row['user_id'] . "_" . $row['photo_id'] . "_" . $g['options']['main_users_photo_size'] . ".jpg";
