...
class CAdminLang extends CHtmlBlock {
        ...
        if ($cmd == 'add_lang') {
            ...
            if ($title == '' or is_dir($langPath)) {
                ...
                if(!file_exists($langPath)) {
                    ...
                    @chmod($langPath, 0777);
                    $languageFilePath = $langPath . 'language.php';
                    file_put_contents($languageFilePath, '<?php //language');
                    @chmod($languageFilePath, 0777);
                    $languageIndexFilePath = $langPath . 'index.html';
                    file_put_contents($languageIndexFilePath, '');
                    @chmod($languageIndexFilePath, 0777);
                }
            ...
            $rtl = get_param('rtl');
            if ($rtl) {
                if (Common::isLanguageFileExists($rtl, $part)) {
                    if (!self::rtlOnLanguage($rtl, $part)) {
                        Config::add('rtl_language_' . $part, $rtl, 1, 'max', 0);
                    }
                }
            }
            redirect("$p?part=$part");
            ...
            $filename = $langDir . $langPart . '/' . get_param('lang') . '/language.php';
    ...
    function parseBlock(&$html)
    {
        ...
        $langs = Common::sortingLangsList($langs, $part);
        ...
        $isRtlLang = Common::isOptionActiveTemplate('rtl_style') && in_array($part, array('main', 'mobile'));
        if ($isRtlLang) {
            $html->parse('lang_rtl_title', false);
        }
        foreach ($langs as $v => $k) {
            $i++;
            if ($i % 2 == 0) {
            ...
            $filename = $langPath = $langDir . $langPart . '/' . $v . '/language.php';
            if(adminFileBackupExists($filename)) {
                $html->parse('lang_restore', false);
            } else {
                $html->setblockvar('lang_restore', '');
            }

            if ($isRtlLang) {
                $rtl = Common::getOption($v, 'rtl_language_' . $part);
                if ($rtl) {
                    $html->parse('lang_rtl_off', false);
                    $html->clean('lang_rtl_on');
                } else {
                    $html->parse('lang_rtl_on', false);
                    $html->clean('lang_rtl_off');
                }
                $html->parse('lang_rtl', false);
            }
    ...
    function rtlOnLanguage($lang, $part = 'main')
    {
        if ((Common::getOption($lang, 'rtl_language_' . $part))) {
            Config::remove('rtl_language_' . $part, $lang);
            return true;
        }
        return false;
    }
