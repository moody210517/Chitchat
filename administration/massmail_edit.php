...
class CAdminMail extends CHtmlBlock
{
	...
	function action()
	{
		...
		if ($cmd == "add_file")
		{
			...
			{
				$mail = file($_FILES[$name]["tmp_name"], FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
                if(count($mail)) {
                    foreach ($mail as $k => $v)
                    {
                        $v = substr($v, 0, 100);
                        ...
                        if ($id == "0" and trim($v) != "")
                        {
                            DB::execute("INSERT IGNORE INTO email (mail) VALUES(" . to_sql(trim($v), "Text") . ")");
                        }
                    }
		...
		elseif ($cmd == "add")
		{
			...
			if ($id == "0")
			{
				DB::execute("INSERT IGNORE INTO email (mail) VALUES(" . to_sql($mail, "Text") . ")");
