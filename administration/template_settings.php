...
class CAdminTemplateEditSettings extends CHtmlBlock
{
    ...
    static function getSettings($tmpl){
        ...
        if ($tmpl == 'edge') {
            ...
            $menuSettings = array(
                'general_settings' => array('type' => 'config'),

                'color_scheme_general' => array('type' => 'config'),
                ...
                'gallery_settings' => array('type' => 'config'),
                'wall_settings' => array('type' => 'config')
            );
        }
	...
	function parseBlock(&$html)
	{
        ...
        /* Menu settings */
        $menuSettingsTemplate = self::getSettings($tmpl);
        ...
        foreach ($menuSettings as $key => $type) {
...
$tmpl = get_param('tmpl');
$menuSettingsTemplate = CAdminTemplateEditSettings::getSettings($tmpl);
$menuSettings = $menuSettingsTemplate['settings'];
$area = get_param('area', $menuSettingsTemplate['default']);
