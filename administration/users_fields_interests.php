...
class CInterestsResults extends CHtmlList
{
	...
	{
        $selectLang = get_param('lang', Common::getOption('main', 'lang_value'));
        $listLangs = Common::listLangs('main', false);
        $sql = 'SELECT `lang` FROM `interests` GROUP BY `lang`';
        $langsInterest = DB::rows($sql);
        $langsTemp = array();
        foreach ($langsInterest as $lang) {
            $langsTemp[$lang[0]] = 1;
        }
        $langsInterest = array();
        foreach ($listLangs as $key => $langTitle) {
            if (isset($langsTemp[$key])) {
                $langsInterest[$key] = $langTitle;
                unset($langsTemp[$key]);
            }
        }
        if ($langsTemp) {
            foreach ($langsTemp as $key => $langTitle) {
                $langKey = $key;
                if($key == 'default') {
                    $langKey = 'language_default';
                }
                $langsInterest[$key] = l(ucfirst($langKey));
            }
        }
        $langsInterest = h_options($langsInterest, $selectLang);
        $html->setvar('select_options_language_list', $langsInterest);
        $html->setvar('select_options_language_add_interest', adminLangsSelect('main', $selectLang));
