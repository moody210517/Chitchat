...
function do_action()
{
	...
	{
        $comment = DB::row("SELECT * FROM events_event_comment WHERE comment_id=" . to_sql($comment_id, 'Number') . " LIMIT 1");
        if($comment)
        {
            DB::execute("INSERT INTO events_event_comment_comment SET parent_comment_id=".$comment['comment_id'].
            ...
            Wall::setSiteSectionItemId($comment['event_id']);
            Wall::add('event_comment_comment', $id);
