...
//if (isset($_FILES['Filedata']) && is_uploaded_file($_FILES['Filedata']["tmp_name"]))
    //{
        ...
        if($file)
        {
            ...
            include('./_include/current/vids/tools.php');
            $r=VideoUpload::upload(0, sanitize_upload_name_all($file));

            // wait convertation
            #@set_time_limit(1000);
            #while(!file_exists($file_name.".flv") && !file_exists($file_name.".jpg")) sleep(1);
            if($r===true){
                die('ok');
            } else {
                if($r=='error_license'){
...
function cleanup_upload_folder_all()
    {
        ...
        foreach($files as $file)
        {
            if($file != '.' && $file != '..' && $file != '.svn' && $file != 'index.html' && $file != '.htaccess')
            {
                ...
                {
                    if(time() - filemtime($file_path) > 24 * 60 * 60) @unlink($file_path);
                }
            }
        }
    }
