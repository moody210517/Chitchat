...
function do_action()
{
	...
	{
        $comment = DB::row("SELECT * FROM groups_group_comment WHERE comment_id=" . to_sql($comment_id, 'Number') . " LIMIT 1");
        if($comment)
        {
	        $group = DB::row("SELECT * FROM groups_group WHERE group_id=" . to_sql($comment['group_id'], 'Number') . " LIMIT 1");
        	if($group && ($group['user_id'] == $g_user['user_id'] || CGroupsTools::is_group_member($group['group_id'])))
	        {
	            DB::execute("INSERT INTO groups_group_comment_comment SET parent_comment_id=".$comment['comment_id'].
                ...
                Wall::add('group_wall_comment', $id);
