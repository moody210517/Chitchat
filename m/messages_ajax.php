...
class CChatAjax extends CHtmlBlock
{
	function init()
	{
		...
		$this->m_on_page = 11;
		$this->m_sql_count = "SELECT count(id) FROM flashchat_messages WHERE room=1 ORDER BY id ASC, time ASC";
		$this->m_sql = "SELECT * FROM flashchat_messages WHERE room=1 ORDER BY id ASC, time ASC";
		//$this->m_debug = "Y";
	}
