...
class CMusic extends CHtmlBlock {
    ...
    function action() {
        ...
        if ($cmd == 'save') {
            ...
            $song_title = get_param('song_title');
            ...
            if ($musician_id && $song_title &&  $song_year && $upload_name) {
                $song = null;
                $length = 0;
                if(file_exists($g['path']['dir_files'] . "music/tmp/" . $upload_name . ".mp3")) {
                    $errorReportingValue = error_reporting(0);
                    global $ioncubeVersionFilename;
                    include_once(dirname(__FILE__) . "/_include/core/mp3reader" . $ioncubeVersionFilename . ".php");
                    $mp3 = new mp3();
                    $length = $mp3->get_length($g['path']['dir_files'] . "music/tmp/" . $upload_name . ".mp3");
