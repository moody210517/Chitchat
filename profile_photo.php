...
class CPhoto extends CHtmlBlock
{
	...
	function action()
	{
		...
		if ($cmd == "insert")
		{
			...
			if ($this->message == "")
			{
				...
				uploadphoto($g_user['user_id'], $photo_name, $description, $g['options']['photo_approval']=='Y'?0:1);
                redirect();
			}
		}
	...
	function parseBlock(&$html)
	{
		...
		DB::query("SELECT * FROM photo WHERE user_id=" . $g_user['user_id'] . "  AND visible != 'P' ORDER BY photo_id ASC");
...
g_user_full();
$page = new CPhoto("", $g['tmpl']['dir_tmpl_main'] . "profile_photo.html");
